...
func crawler(keyword string, xc int, page int) {
	...
	for page <= 25 {
		...
		if len(fk) > 0 {
			log.Println("被风控，更换IP重新开始,Robot")
			IsC2 = !isc2
			continue
		}
		...
		//log.Println(result)
		resultStr := ""
		resultS := regexp.MustCompile("(\"paginationV2\":{.+?\"requestContext\")").FindAllStringSubmatch(result, -1)
		...
		if len(resultS) == 0 {
			err := ioutil.WriteFile("1.txt", []byte(result), 0644)
			if err != nil {
				fmt.Println("写入文件时出错:", err)
				return
			}

			fmt.Println("变量的值已成功写入到 1.txt 文件")
			log.Println("被风控，更换IP重新开始,resultS==0")
			continue
