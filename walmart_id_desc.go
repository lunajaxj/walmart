...
func main() {
	log.Println("自动化脚本-walmart-id获取产品文案")
	log.Println("开始执行...")
	// 创建句柄
	...
	num := 2
	if err := xlsx.SetSheetRow("Sheet1", "A1", &[]interface{}{"id", "文案"}); err != nil {
		log.Println(err)
	}
	for _, sv := range ids {
		for _, v := range res {
			if v.id == sv {
				if err := xlsx.SetSheetRow("Sheet1", "A"+strconv.Itoa(num), &[]interface{}{v.id, v.desc}); err != nil {
					log.Println(err)
				}
				num++
			}
...
func crawler(u string) {
	...
	for i := 0; i < 16; i++ {
		...
		request, _ := http.NewRequest("PUT", "https://www.walmart.com/ip/"+u, nil)
		...
		desc := regexp.MustCompile(`"shortDescription":"(.*)?","fulfillmentType`).FindAllStringSubmatch(result, -1)
		if len(desc) > 0 {
			regex := regexp.MustCompile(`\\u[0-9a-fA-F]{4}`) // 匹配Unicode编码的正则表达式

			cleanString := regex.ReplaceAllString(desc[0][1], "")
			wal.desc = cleanString
		} else {
			wal.desc = "无"
		}
		if len(fk) > 0 {
