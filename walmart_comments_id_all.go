...
func main() {
	log.Println("自动化脚本-walmart-itemid获取全部评论信息")
	log.Println("开始执行...")
	// 创建句柄
...
func save(keys string) {
	...
	if err := xlsx.SetSheetRow("Sheet1", "A1", &[]interface{}{"item id", "评论总数", "文字数量", "纯星级数量", "一星数量", "二星数量", "三星数量", "四星数量", "五星数量"}); err != nil {
		log.Println(err)
	}
	num := 0
	for i := range ids {
	...
	if res[keys].id != "" {
		if err := xlsx.SetSheetRow("Sheet1", "A"+strconv.Itoa(num), &[]interface{}{res[keys].id, res[keys].ratingNum, res[keys].wzRating, res[keys].xjRating, res[keys].rating1, res[keys].rating2, res[keys].rating3, res[keys].rating4, res[keys].rating5}); err != nil {
			log.Println(err)
		}
	}
...
func crawler(id string) {
	...
	for i := 1; i <= 25; {
		...
		idsz := regexp.MustCompile(`"reviewId":"(.+?)",`).FindAllStringSubmatch(result, -1)
		var indexs []int
	fo1:
		for i2 := range idsz {
			for i3 := range idds {
				if idds[i3] == idsz[i2][1] {
					continue fo1
				}
			}
			idds = append(idds, idsz[i2][1])
			indexs = append(indexs, i2)
		}

		wzRation := regexp.MustCompile(`"reviewText":(.+?),`).FindAllStringSubmatch(result, -1)
		if len(wzRation) >= len(indexs) {
			for i2 := range indexs {
				mode.ratingNum += 1
				switch wzRation[i2][1] {
				case "null":
					mode.xjRating += 1
				default:
					mode.wzRating += 1
		...
		ration := regexp.MustCompile(`"rating":(\d),`).FindAllStringSubmatch(result, -1)

		if len(ration) >= len(indexs) {
			for i2 := range indexs {
				switch ration[i2][1] {
				case "1":
					mode.rating1 += 1
