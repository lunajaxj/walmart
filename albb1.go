...
func forr() chromedp.ActionFunc {
	...
	`, baseSelector, baseSelector)
			...
			err = chromedp.Run(newCtx, chromedp.Location(&url)) // 等待5秒
			if err != nil {
			...
			// JavaScript 代码，返回元素的文本内容
			jsScript = `(() => {
		const element = document.querySelector('#content > div > div > div:nth-child(9) > div > div > div.content.tab-content.tab-content-notab > div > div.step-detail > div.step-detail-header > div > em');
		return element ? element.textContent : 'Element not found';
	})()`

			// 变量用来存储执行 JavaScript 后的结果
			var result1 string

			// 执行 ChromeDP 任务
			err := chromedp.Run(ctx,
				chromedp.Evaluate(jsScript, &result1),
			)
			if err != nil {
			...
			// 打印获取的结果
			fmt.Println("Text from selector:", result1)
			//log.Println(pageSource)
			//var buf []byte
			//err = chromedp.Run(newCtx,
			//	chromedp.Screenshot("html", &buf, chromedp.ByQuery),
			//)
			//log.Println(buf)
			//if err != nil {
			//	fmt.Println("erro screenShot: %v", err)
			//}
			//err = ioutil.WriteFile(".\\img\\"+cgds[i1]+".jpg", buf, 0644)
			//if err != nil {
