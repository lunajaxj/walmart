...
func main() {
	log.Println("自动化脚本-walmart-卖家库存信息采集")
	log.Println("开始执行...")
	// 创建句柄
	...
	num := 2
	if err := xlsx.SetSheetRow("Sheet1", "A1", &[]interface{}{"id", "卖家", "配送", "库存"}); err != nil {
		log.Println(err)
	}
	for _, sv := range ids {
		for _, v := range res {
			if v.id == sv {
				if err := xlsx.SetSheetRow("Sheet1", "A"+strconv.Itoa(num), &[]interface{}{v.id, v.seller, v.delivery, v.inventory}); err != nil {
					log.Println(err)
				}
				num++
