...
func save(keys string) {
	...
	defer xlsx.Close()
	if count == 0 {
		if err := xlsx.SetSheetRow("Sheet1", "A1", &[]interface{}{"链接", "图片", "id", "商品码类型", "商品码值", "品牌", "标签", "标题", "评分", "评论数量", "价格", "卖家", "配送", "变体1", "变体2", "变体id", "到达时间"}); err != nil {
			log.Println(err)
		}
	}
	count += 2
	for _, v := range res[keys] {
		...
		if err := xlsx.SetSheetRow("Sheet1", "A"+strconv.Itoa(count), &[]interface{}{v.url, nil, v.id, v.typez, v.value, v.brand, v.query, v.title, v.score, v.review, v.price, v.seller, v.delivery, v.variant1, v.variant2, other, v.deliveryDate}); err != nil {
			log.Println(err)
		}
		if err := xlsx.AddPicture("Sheet1", "B"+strconv.Itoa(count), v.img, nil); err != nil {
			log.Println(err)
		}
		count++
	}
...
func imgxzz(id, img string) {
	...
	for i := 0; i < 5; i++ {
		...
		proxyUrl, _ := url.Parse("http://l752.kdltps.com:15818")
		tr.Proxy = http.ProxyURL(proxyUrl)
		basicAuth := "Basic " + base64.StdEncoding.EncodeToString([]byte("t19932187800946:m78z02hx"))
		tr.ProxyConnectHeader = http.Header{}
		tr.ProxyConnectHeader.Add("Proxy-Authorization", basicAuth)
